...
public class Assign04TimingTests {
    ...
    public static String findKOurs(int sizeOfList, int timesToLoop) {
    ...
    public static String findKJavas(int sizeOfList, int timesToLoop) {
        ...
        Long startTime = System.nanoTime();
        for (int timesLooped = 0; timesLooped < timesToLoop; timesLooped++) {
            LargestNumberSolver.findKthLargestJava(list,0);
        }
        Long midPointTime = System.nanoTime();
        for (int timesLooped = 0; timesLooped < timesToLoop; timesLooped++) {
    ...
    public static void generateData(int sizeN, int timesToLoop, int increment, String File) {
        ...
        System.out.println("Data Collection for Assignment 4.");
        System.out.println(separate);

        ArrayList<String[]> data = new ArrayList<>();
        System.out.println("Collecting Data with Parameters:");
        String[] parameters = {"SizeN", "findKLargestOurs time(ns)", "findKLargestJavas time(ns)"};
        data.add(parameters);
        ...
        for (int size = 1; size <= sizeN; size += increment) {
            ...
            String ours = findKOurs(size,timesToLoop);
            String javas = findKJavas(size,timesToLoop);
