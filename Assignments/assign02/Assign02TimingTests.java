...
public class Assign02TimingTests {
...
//    public Assign02TimingTests(){}
    ...
    public static String lookupTiming(int sizeOfList,int timesToLoop){
        CS2420Class students = new CS2420Class();

        for(int numStudents = 0; numStudents < sizeOfList; numStudents++){
            String emailAddress = Integer.toString(numStudents) + '.' + Integer.toString(numStudents);
            students.addStudent(new CS2420Student(Integer.toString(numStudents),Integer.toString(numStudents),numStudents,new EmailAddress(emailAddress,"@gmail.com")));
        }

        Long startTime = System.nanoTime();
        for(int timesLooped = 0; timesLooped < timesToLoop; timesLooped++){

            students.lookup(sizeOfList - 1);
        }
    ...
    public static void generateData(int sizeN,int timesToLoop, int increment,String File){
        ...
        System.out.println("Data Collection for lookupTiming.");
        System.out.println(separate);

        ArrayList<String[]> data = new ArrayList<>();
        System.out.println("Collecting Data with Parameters:");
        String[] parameters = {"SizeN", "lookUp(uuid) time (ns)"};
        data.add(parameters);
        ...
        for(int size = 1; size <= sizeN; size+= increment){
            ...
            String lookUp = lookupTiming(size,timesToLoop);

            String[] dataPoint = new String[]{N,lookUp};
            if(size == 1){
                size--;
            }
            data.add(dataPoint);
